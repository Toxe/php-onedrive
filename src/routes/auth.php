<?php
declare(strict_types=1);

namespace PHPOneDrive\Route;

require_once(__DIR__ . "/../onedrive.php");

function handle_GET_request(string $request_uri): \PHPOneDrive\RequestResult
{
    // If we don't have a code in the query string (meaning that the user did not log in successfully
    // or did not grant privileges requested), we cannot proceed in obtaining an access token.
    if (!array_key_exists('code', $_GET))
        return \PHPOneDrive\Content::error("Authorization failed: Missing code.")->result();

    if (!($client = \PHPOneDrive\restore_onedrive_client_from_session()))
        return \PHPOneDrive\Content::error("Authorization failed: Unable to restore client state.")->result();

    \PHPOneDrive\obtain_onedrive_access_token($client, $_GET['code']);
    \PHPOneDrive\save_onedrive_user_info_to_session($client);
    \PHPOneDrive\save_onedrive_client_state_to_session($client);

    return \PHPOneDrive\RequestResult::redirect("/drive");  // redirect to /drive
}
